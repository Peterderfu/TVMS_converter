...
ls $NTP
echo "{----}"
echo "{--OS info--}" && lsb_release -a && ip address|grep "inet "; echo "{----}"
...
echo "{--03--}" && grep -E "/var/log/messages|/var/log/secure|/var/log/maillog|/var/log/spooler|/var/log/cron" /etc/logrotate.d/rsyslog|while read -r item;do printf "$item|";done|sed 's/|$//g';echo;echo "{----}"
echo "{--04--}" && ls –la /etc/cron.daily/0logwatch |grep "0logwatch->/usr/share/logwatch/scripts/logwatch.pl"|wc -l;echo "{----}"
...
echo "{--18--}" && cat $PASSWORD_POLICY|grep "pam_unix.so"|grep sha512;echo "{----}"
echo "{--19--}" && cat $LOGIN|grep "pam_tally2.so"|grep deny;echo "{----}"
echo "{--20--}" && dpkg -s auditd ; ls /etc/rc*.d/S*auditd;echo "{----}"
echo "{--21--}" && cat $AUDIT_RULE|grep "\-F arch=b`uname -m|awk -F_ '{print $2}'`"|grep -E "\-k time-change";echo "{----}"
...
echo "{--36--}" && ufw status;echo "{----}"
echo "{--37--}" && iptables -L;echo "{----}"
