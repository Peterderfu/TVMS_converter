...
def getAudit10(match):
    return evalFunc2('(?<=^minlen)[\b\s]*\d+',match.group(0).strip('\n').replace('=',''),MINLEN)
...
def getAudit17(match):
    patterns = ["/etc/pam.d/system-auth:\s*password\s+(requisite|required|sufficient)\s+pam_(pwhistory|unix)\.so.+remember=([5-9]|[1-9][0-9]+)", \
                "/etc/pam.d/password-auth:\s*password\s+(requisite|required|sufficient)\s+pam_(pwhistory|unix)\.so.+remember=([5-9]|[1-9][0-9]+)"]
    return evalFunc1(patterns, match.group(0).strip('\n'))
def getAudit18(match):
    patterns = ["/etc/pam.d/password-auth:password\s+(requisite|required|sufficient)\s+pam_unix\.so.+sha512",\
                "/etc/pam.d/system-auth:password\s+(requisite|required|sufficient)\s+pam_unix\.so.+sha512"]
    return evalFunc1(patterns,match.group(0).strip('\n'))
